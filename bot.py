...
# ========== ПЕРЕВОДЫ (сокращённо, но с основными фразами) ==========
TRANSLATIONS = {
    'ru': {
...
async def lang_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # После сохранения языка перезаписываем сообщение на выбранном языке
    await show_welcome(update, context, lang)
...
async def show_welcome(update: Update, context: ContextTypes.DEFAULT_TYPE, lang):
    ...
    # Проверяем, откуда пришёл вызов — из callback или нового сообщения
    if update.callback_query:
        ...
        try:
            ...
            # Если не получилось отредактировать (например, сообщение уже изменено), просто отправляем новое
        ...
        # Если это новый запрос (команда /start), отправляем новое сообщение
        await update.message.reply_text(
            welcome_text,
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="HTML"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang = get_user_lang(user_id)
    if lang is None:
        # Если язык не выбран, показываем меню выбора языка
        keyboard = [
            [InlineKeyboardButton("🇷🇺 Русский", callback_data="lang_ru")],
            [InlineKeyboardButton("🇬🇧 English", callback_data="lang_en")],
            [InlineKeyboardButton("🇸🇦 العربية", callback_data="lang_ar")],
