...
# ========== РАБОТА С ЯЗЫКОМ (надёжное хранение в файле) ==========
LANG_FILE = "user_lang.json"

def get_user_lang(user_id):
    """Возвращает язык пользователя или None, если не выбран."""
    try:
...
def set_user_lang(user_id, lang):
    """Сохраняет язык пользователя в файл."""
    try:
        with open(LANG_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
...
# ========== ПЕРЕВОДЫ ==========
# Здесь только основные фразы, чтобы не перегружать код.
# Ты можешь расширить словарь позже.
TRANSLATIONS = {
    'ru': {
