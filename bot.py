...
def get_time_left():
    ...
    now = datetime.now()
    if now >= end_date:
        return "❌ Акция завершена! / Discount ended! / انتهت الخصم! / 割引終了！"
    diff = end_date - now
    days = diff.days
...
def format_price(rub_price, lang, ton_price):
    if lang == 'ru':
        return f"{rub_price} ₽ / {ton_price} TON"
    else:
        usd = int(rub_price.replace(' ', '').replace('₽', '').strip())
        usd = round(usd / 90, 2)
        return f"${usd} / {ton_price} TON"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not lang:
        ...
        await update.message.reply_text(TRANSLATIONS['ru']['select_lang'], reply_markup=InlineKeyboardMarkup(keyboard))
        return
    # Если язык уже выбран, показываем приветствие
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "show_categories":
        ...
        header = t['choose_category'].format(time_left=time_left) if discount_active else "📂 <b>Выбери категорию:</b>"
    ...
    if data.startswith("cat_"):
        ...
        for prod_id, info in items.items():
            # Формируем цену с учётом языка
            rub_price = info['price'].split(' / ')[0]  # часть с рублями
            ton_price = info['price'].split(' / ')[1] if ' / ' in info['price'] else ''
            price_display = format_price(rub_price, lang, ton_price)
            if DISCOUNT:
                # применить скидку к рублям и тонам
                discounted_rub = apply_discount(rub_price, DISCOUNT)
                discounted_ton = apply_discount(ton_price, DISCOUNT) if ton_price else ''
                price_display = format_price(discounted_rub, lang, discounted_ton)
    ...
    if data.startswith("prod_"):
        ...
        if found:
            ...
            rub_price = found['price'].split(' / ')[0]
            ton_price = found['price'].split(' / ')[1] if ' / ' in found['price'] else ''
            price_display = format_price(rub_price, lang, ton_price)
            if DISCOUNT:
                discounted_rub = apply_discount(rub_price, DISCOUNT)
                discounted_ton = apply_discount(ton_price, DISCOUNT) if ton_price else ''
                price_display = format_price(discounted_rub, lang, discounted_ton)
            # Для арабского добавляем предупреждение
