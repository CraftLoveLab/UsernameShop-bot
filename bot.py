...
async def show_welcome(update: Update, context: ContextTypes.DEFAULT_TYPE, lang):
    ...
    if update.callback_query:
        await safe_edit(update, welcome_text, reply_markup=InlineKeyboardMarkup(keyboard))
    else:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lang is None:
        ...
        await safe_edit(update, "🌐 Выберите язык / Choose language / اختر اللغة / 言語を選択してください:", reply_markup=InlineKeyboardMarkup(keyboard))
        return
    t = TRANSLATIONS[lang]
    ...
    if data == "why_price":
        await safe_edit(update, t['why_price'], reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t['back_to_categories'], callback_data="show_categories")]]))
        return
    ...
    if data == "show_categories":
        ...
        if discount_active:
            ...
            header = "📂 <b>Выбери категорию:</b>"
        await safe_edit(update, header, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    if data.startswith("cat_"):
        ...
        await safe_edit(update, t['category_items'].format(category=category), reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    if data.startswith("prod_"):
        ...
        if found:
            ...
            await safe_edit(update, "❌ Товар не найден.")
        return
    ...
    if data.startswith("contact_"):
        ...
        if found:
            ...
            await safe_edit(update, t['contact_admin'].format(contact=CONTACT_USERNAME))
    ...
    if data.startswith("spam_"):
        ...
        await safe_edit(update, t['spam_block'])
        return
...
def main():
    ...
    try:
        ...
        # Повторный запуск через 2 секунды, если ошибка Conflict
        import asyncio
        import time
        time.sleep(2)
